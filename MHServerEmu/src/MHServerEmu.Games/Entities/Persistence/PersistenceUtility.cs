    ...
    public static class PersistenceUtility
    {
        ...
        private static bool RestoreContainer(Entity container, DBEntityCollection entities)
        {
            ...
            for (int i = 0; i < dbEntityList.Count; i++)
            {
                ...
                if (!Verify.IsTrue(entityProtoRef != PrototypeId.Invalid, LoggingLevel.Error,
                        $"Failed to retrieve entity proto ref for guid {dbEntity.EntityProtoGuid}. " +
                        $"This item will be DELETED on the next save. If it is a custom costume token, " +
                        $"its costume was uninstalled without a purge; remove the row with: " +
                        $"DELETE FROM Item WHERE DbGuid = {dbEntity.DbGuid};"))
                    continue;
                ...
                if (entityManager.CreateEntity(settings) == null)
                {
                    Logger.Error($"RestoreContainer(): could NOT restore item DbGuid={dbEntity.DbGuid} " +
                                 $"(container={dbEntity.ContainerDbGuid}, entityProtoGuid={dbEntity.EntityProtoGuid}). " +
                                 $"It is skipped every login. If it never clears, remove it with: " +
                                 $"DELETE FROM Item WHERE DbGuid = {dbEntity.DbGuid};");
                }
            }
