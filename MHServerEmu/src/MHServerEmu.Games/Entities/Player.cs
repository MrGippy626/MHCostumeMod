...
namespace MHServerEmu.Games.Entities
{
    ...
    public partial class Player : Entity, IMissionManagerOwner
    {
        ...
        /// <summary>
        /// </summary>
        public bool RefreshCurrentAvatar()
        {
            Avatar avatar = CurrentAvatar;
            if (avatar == null || avatar.IsInWorld == false)
            {
                Logger.Info($"[CostumeRefresh] SKIPPED avatar={(avatar == null ? "null" : avatar.ToString())} IsInWorld={(avatar?.IsInWorld.ToString() ?? "n/a")}");
                return false;
            }

            Logger.Info($"[CostumeRefresh] START avatar=[{avatar}] costume=0x{(ulong)avatar.EquippedCostumeRef:X} - re-realizing in place");

            ulong regionId = avatar.RegionLocation.RegionId;
            Vector3 position = avatar.RegionLocation.Position;
            Orientation orientation = avatar.RegionLocation.Orientation;
            ulong avatarId = avatar.Id;

            avatar.ExitWorld();

            avatar.SuppressCustomCostumeReassert();

            bool ok = EnableCurrentAvatar(false, avatarId, regionId, position, orientation);
            Logger.Info($"[CostumeRefresh] DONE ok={ok}");
            return ok;
        }
