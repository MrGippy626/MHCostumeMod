...
namespace MHServerEmu.Games.GameData.Calligraphy
{
    ...
    public class Blueprint
    {
        ...
        /// <summary>
        /// </summary>
        public bool InjectCustomEnum(PrototypeId customId, int enumValue)
        {
            if (_prototypeToEnumValueLookup == null || enumValue <= 0)
                return false;

            if (_prototypeToEnumValueLookup.TryGetValue(customId, out int existing))
                return existing == enumValue;

            if (enumValue >= _enumValueToPrototypeLookup.Length)
            {
                PrototypeId[] grown = new PrototypeId[enumValue + 1];
                Array.Copy(_enumValueToPrototypeLookup, grown, _enumValueToPrototypeLookup.Length);
                _enumValueToPrototypeLookup = grown;
            }
            else if (_enumValueToPrototypeLookup[enumValue] != PrototypeId.Invalid)
            {
                return false;   // slot belongs to real content - caller logs and skips
            }

            _enumValueToPrototypeLookup[enumValue] = customId;
            _prototypeToEnumValueLookup[customId] = enumValue;
            return true;
        }
