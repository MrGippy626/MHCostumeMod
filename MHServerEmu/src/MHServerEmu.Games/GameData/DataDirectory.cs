...
namespace MHServerEmu.Games.GameData
{
    ...
    public sealed class DataDirectory
    {
        ...
        private readonly Dictionary<PrototypeId, PrototypeGuid> _customCostumeGuids = new();

        private const ulong CustomCostumeGuidSalt = 0xC057_11E5_C057_11E5;

        /// <summary>
        /// </summary>
        public static ulong ForgeCustomCostumeGuid(ulong customId) => customId ^ CustomCostumeGuidSalt;
        ...
        /// <summary>
        /// </summary>
        public void EnsureGlobalEnumCapacity(int requiredSize)
        {
            if (_prototypeEnumValueLookupByClassType.TryGetValue(typeof(Prototype), out var globalLookup) == false)
            {
                Logger.Warn("EnsureGlobalEnumCapacity: Global lookup not found.");
                return;
            }

            if (globalLookup.EnumValueToPrototypeLookup == null || requiredSize >= globalLookup.EnumValueToPrototypeLookup.Length)
            {
                int newSize = requiredSize + 1;
                PrototypeId[] newArray = new PrototypeId[newSize];
                if (globalLookup.EnumValueToPrototypeLookup != null)
                    Array.Copy(globalLookup.EnumValueToPrototypeLookup, newArray, globalLookup.EnumValueToPrototypeLookup.Length);
                else
                    newArray[0] = PrototypeId.Invalid;

                globalLookup.EnumValueToPrototypeLookup = newArray;
            }
        }

        /// <summary>
        /// </summary>
        public void InjectGlobalEnum(PrototypeId customId, int targetEnum)
        {
            if (_prototypeEnumValueLookupByClassType.TryGetValue(typeof(Prototype), out var globalLookup) == false)
            {
                Logger.Warn("InjectGlobalEnum: Global lookup not found.");
                return;
            }

            int existingLength = globalLookup.EnumValueToPrototypeLookup?.Length ?? 0;
            if (targetEnum < existingLength)
            {
                PrototypeId occupant = globalLookup.EnumValueToPrototypeLookup[targetEnum];
                if (occupant != PrototypeId.Invalid && occupant != customId)
                {
                    Logger.Warn($"InjectGlobalEnum: enum {targetEnum} is ALREADY USED by real content " +
                                $"[{GameDatabase.GetPrototypeName(occupant)}] (0x{(ulong)occupant:X}) - " +
                                $"custom 0x{(ulong)customId:X} is DISPLACING it. Global enum count is " +
                                $"{existingLength}; custom enums must start above that.");
                }
            }

            EnsureGlobalEnumCapacity(targetEnum);

            globalLookup.EnumValueToPrototypeLookup[targetEnum] = customId;
            globalLookup.PrototypeToEnumValueLookup[customId] = targetEnum;

            Logger.Info($"InjectGlobalEnum: Set custom ID 0x{(ulong)customId:X} to global enum {targetEnum} " +
                        $"(global enum count was {existingLength})");
        }

        /// <summary>
        /// </summary>
        public bool InjectClassEnum<T>(PrototypeId customId, int targetEnum) where T : Prototype
        {
            if (_prototypeEnumValueLookupByClassType.TryGetValue(typeof(T), out var lookup) == false)
            {
                Logger.Warn($"InjectClassEnum: no lookup for {typeof(T).Name}");
                return false;
            }

            if (lookup.PrototypeToEnumValueLookup.TryGetValue(customId, out int existing))
                return existing == targetEnum;

            if (lookup.EnumValueToPrototypeLookup == null || targetEnum >= lookup.EnumValueToPrototypeLookup.Length)
            {
                PrototypeId[] grown = new PrototypeId[targetEnum + 1];
                if (lookup.EnumValueToPrototypeLookup != null)
                    Array.Copy(lookup.EnumValueToPrototypeLookup, grown, lookup.EnumValueToPrototypeLookup.Length);
                else
                    grown[0] = PrototypeId.Invalid;
                lookup.EnumValueToPrototypeLookup = grown;
            }
            else if (lookup.EnumValueToPrototypeLookup[targetEnum] != PrototypeId.Invalid)
            {
                Logger.Warn($"InjectClassEnum: enum {targetEnum} in {typeof(T).Name} already belongs to " +
                            $"0x{(ulong)lookup.EnumValueToPrototypeLookup[targetEnum]:X} - NOT overwriting");
                return false;
            }

            lookup.EnumValueToPrototypeLookup[targetEnum] = customId;
            lookup.PrototypeToEnumValueLookup[customId] = targetEnum;
            return true;
        }

        /// <summary>
        /// </summary>
        private void InjectBlueprintEnumRecursive(BlueprintId blueprintId, PrototypeId customId, int targetEnum,
                                                  HashSet<BlueprintId> visited = null)
        {
            if (blueprintId == BlueprintId.Invalid) return;

            visited ??= new HashSet<BlueprintId>();
            if (visited.Add(blueprintId) == false) return;

            if (_loadedBlueprints.TryGetValue(blueprintId, out var record) == false) return;

            if (record.Blueprint.InjectCustomEnum(customId, targetEnum) == false)
                Logger.Warn($"InjectCustomCostume: blueprint enum injection refused for " +
                            $"0x{(ulong)customId:X} in blueprint {blueprintId} (slot taken?)");

            BlueprintId[] parents = record.Blueprint.Parents;
            if (parents == null) return;

            foreach (BlueprintId parent in parents)
                InjectBlueprintEnumRecursive(parent, customId, targetEnum, visited);
        }
        ...
        /// <remarks>
        /// </remarks>
        public bool InjectCustomHotspot(PrototypeId forgedId, PrototypeId stockId, int targetEnum)
        {
            if (forgedId == PrototypeId.Invalid || stockId == PrototypeId.Invalid)
                return false;

            if (!_prototypeDataRefRecords.TryGetValue(stockId, out var stockRecord))
            {
                Logger.Warn($"InjectCustomHotspot: stock hotspot 0x{(ulong)stockId:X} has no record - " +
                            "cannot alias; this costume's hotspots will render stock.");
                return false;
            }

            if (_prototypeDataRefRecords.TryGetValue(forgedId, out var existing))
            {
                if (ReferenceEquals(existing, stockRecord))
                    return true;

                Logger.Warn($"InjectCustomHotspot: forged id 0x{(ulong)forgedId:X} is already taken " +
                            "by a different prototype - refusing to alias over it. Its costume's " +
                            "hotspots will render stock.");
                return false;
            }

            InjectGlobalEnum(forgedId, targetEnum);

            if (InjectClassEnum<EntityPrototype>(forgedId, targetEnum) == false)
                Logger.Warn($"InjectCustomHotspot: EntityPrototype enum injection failed for " +
                            $"0x{(ulong)forgedId:X} - the hotspot will not replicate.");

            InjectBlueprintEnumRecursive(stockRecord.BlueprintId, forgedId, targetEnum);

            _prototypeDataRefRecords[forgedId] = stockRecord;
            return true;
        }

        public void InjectCustomCostume(PrototypeId customId, int targetEnum, PrototypeId donorId)
        {
            InjectGlobalEnum(customId, targetEnum);

            if (InjectClassEnum<EntityPrototype>(customId, targetEnum) == false)
                Logger.Warn($"InjectCustomCostume: EntityPrototype enum injection failed for " +
                            $"0x{(ulong)customId:X} - a token item carrying this id will not replicate.");

            if (donorId != PrototypeId.Invalid && _prototypeDataRefRecords.TryGetValue(donorId, out var donorRec))
                InjectBlueprintEnumRecursive(donorRec.BlueprintId, customId, targetEnum);

            if (donorId == PrototypeId.Invalid)
            {
                Logger.Warn($"InjectCustomCostume: no donorId for 0x{(ulong)customId:X} - " +
                            "the costume will work but saving it may fail (no data record).");
                return;
            }

            if (_prototypeDataRefRecords.ContainsKey(customId))
                return;

            if (!_prototypeDataRefRecords.TryGetValue(donorId, out var donorRecord))
            {
                Logger.Warn($"InjectCustomCostume: donor 0x{(ulong)donorId:X} has no record - " +
                            "cannot alias custom id for persistence.");
                return;
            }

            _prototypeDataRefRecords[customId] = donorRecord;

            if (!_customCostumeGuids.ContainsKey(customId))
            {
                PrototypeGuid customGuid = (PrototypeGuid)ForgeCustomCostumeGuid((ulong)customId);

                if (_prototypeGuidToDataRefLookup.ContainsKey(customGuid))
                {
                    Logger.Warn($"InjectCustomCostume: forged guid 0x{(ulong)customGuid:X} for custom " +
                                $"0x{(ulong)customId:X} collides with a real prototype guid - skipping. " +
                                "The costume works; its token will degrade to the donor on relog.");
                }
                else
                {
                    _customCostumeGuids[customId] = customGuid;
                    _prototypeGuidToDataRefLookup[customGuid] = customId;
                }
            }
            Logger.Info($"InjectCustomCostume: aliased custom 0x{(ulong)customId:X} -> " +
                        $"donor record 0x{(ulong)donorId:X} for persistence");
        }
        ...
        public PrototypeGuid GetPrototypeGuid(PrototypeId prototypeDataRef)
        {
            ...
            if (_customCostumeGuids.TryGetValue(prototypeDataRef, out PrototypeGuid customGuid))
                return customGuid;
        ...
        private class PrototypeEnumValueNode
        {
            ...
            public PrototypeId[] EnumValueToPrototypeLookup { get; set; }
            public Dictionary<PrototypeId, int> PrototypeToEnumValueLookup { get; set; }
