...
namespace MHServerEmu.Games.Network
{
    ...
    public static class ArchiveMessageBuilder
    {
        /// <summary>
        /// </summary>
        private static PrototypeId ResolveEntityProtoRefForRecipient(PrototypeId entityProtoRef, PlayerConnection recipient)
        {
            if (CustomCostumeLoader.HasForgedHotspots == false)
                return entityProtoRef;

            PrototypeId stockRef = CustomCostumeLoader.GetStockForForged(entityProtoRef);
            if (stockRef == PrototypeId.Invalid)
                return entityProtoRef;                  // not a forged id - the overwhelming majority

            if (recipient != null && recipient.CanDecodeForgedRef(entityProtoRef))
                return entityProtoRef;                  // this client has the mod: give it the custom FX

            CustomCostumeLoader.ReportForgedRefSubstituted(entityProtoRef, stockRef,
                                                           recipient != null && recipient.HasRegisteredForgedRefs,
                                                           recipient?.SessionId ?? 0);
            return stockRef;
        }
        ...
        /// </summary>
        public static NetMessageEntityCreate BuildEntityCreateMessage(Entity entity, AOINetworkPolicyValues interestPolicies, bool includeInvLoc, EntitySettings settings = null, PlayerConnection recipient = null)
        {
            ...
            using (Archive archive = new Archive(ArchiveSerializeType.Replication, (ulong)interestPolicies))
            {
                ...
                PrototypeId entityPrototypeRef = ResolveEntityProtoRefForRecipient(entity.PrototypeDataRef, recipient);
        ...
        /// </summary>
        public static NetMessageEntityEnterGameWorld BuildEntityEnterGameWorldMessage(WorldEntity worldEntity, EntitySettings settings = null, PlayerConnection recipient = null)
        {
            // Build flags
            LocomotionMessageFlags locoFieldFlags = LocomotionMessageFlags.None;
            ...
            {
                PrototypeId entityPrototypeRef = ResolveEntityProtoRefForRecipient(worldEntity.PrototypeDataRef, recipient);
                Serializer.TransferPrototypeEnum<EntityPrototype>(archive, ref entityPrototypeRef);
            }
