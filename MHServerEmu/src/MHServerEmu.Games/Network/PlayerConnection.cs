...
namespace MHServerEmu.Games.Network
{
    ...
    public class PlayerConnection : NetClient
    {
        ...
        /// <summary>
        /// </summary>
        public bool CustomCostumesEnabled
        {
            get => ((DBPlayerFlags)_dbAccount.Player.Flags).HasFlag(DBPlayerFlags.CustomCostumesEnabled);
            set
            {
                if (value)
                    _dbAccount.Player.Flags |= (long)DBPlayerFlags.CustomCostumesEnabled;
                else
                    _dbAccount.Player.Flags &= ~(long)DBPlayerFlags.CustomCostumesEnabled;
            }
        }
        ...
        /// <remarks>
        /// </remarks>
        public ulong SessionId => _frontendClient?.Session?.Id ?? 0;

        public bool CanDecodeForgedRef(PrototypeId forgedRef)
        {
            if (CustomCostumesEnabled == false)
                return false;

            bool canDecode = CustomCostumeRegistry.CanDecode(SessionId, (ulong)forgedRef, out bool known);

            if (known)
                return canDecode;

            return _requireRegistration == false;
        }

        /// <summary>
        /// </summary>
        public bool HasRegisteredForgedRefs
        {
            get
            {
                CustomCostumeRegistry.CanDecode(SessionId, 0, out bool known);
                return known;
            }
        }
        ...
        /// <summary>
        /// </summary>
        private static readonly bool _requireRegistration =
            ConfigManager.Instance.GetConfig<CustomGameOptionsConfig>().RequireCustomCostumeRegistration;
        ...
        private bool LoadFromDBAccount()
        {
            ...
            CustomCostumeLoader.LoadOverridesForPlayer((ulong)_dbAccount.Id, _dbAccount.Player.CustomCostumes);
        ...
        private bool SaveToDBAccount(bool saveMigrationData)
        {
            ...
            _dbAccount.Player.CustomCostumes = CustomCostumeLoader.SerializeOverridesForPlayer((ulong)_dbAccount.Id);
