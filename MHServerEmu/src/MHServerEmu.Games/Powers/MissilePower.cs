...
namespace MHServerEmu.Games.Powers
{
    public class MissilePower : Power
    {
        ...
        private bool CreateMissile(EntityManager entityManager, MissileCreationContextPrototype missileContext, PowerApplication powerApplication, int contextIndex, int missileIndex)
        {
            ...
            if (CustomCostumeLoader.HasForgedHotspots && Owner is Avatar missileCaster)
            {
                PrototypeId costume = missileCaster.Properties[PropertyEnum.CostumeCurrent];
                PrototypeId forged = CustomCostumeLoader.GetForgedHotspot(costume, creationSettings.EntityRef);

                if (forged != PrototypeId.Invalid)
                {
                    CustomCostumeLoader.ReportForgedSummon(costume, creationSettings.EntityRef,
                                                           forged, PrototypeDataRef);
                    creationSettings.EntityRef = forged;
                }
                else
                    CustomCostumeLoader.ReportUnforgedHotspot(costume, creationSettings.EntityRef,
                                                             PrototypeDataRef);
            }
