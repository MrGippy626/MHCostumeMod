...
namespace MHServerEmu.Games.Powers
{
    public class SummonPower : Power, IPowerPayloadDeliverCallback
    {
        ...
        private static PowerUseResult SummonEntity(EntityManager entityManager, ref SummonContext context, int index)
        {
            ...
            if (CustomCostumeLoader.HasForgedHotspots)
            {
                Avatar caster = ultimateOwner as Avatar ?? owner as Avatar;
                if (caster != null)
                {
                    PrototypeId costume = caster.Properties[PropertyEnum.CostumeCurrent];
                    PrototypeId forged = CustomCostumeLoader.GetForgedHotspot(costume, settings.EntityRef);

                    if (forged != PrototypeId.Invalid)
                    {
                        settings.EntityRef = forged;
                    }
                    else
                    {
                        CustomCostumeLoader.ReportUnforgedHotspot(costume, settings.EntityRef,
                                                                 powerProto.DataRef);
                    }
                }
            }
