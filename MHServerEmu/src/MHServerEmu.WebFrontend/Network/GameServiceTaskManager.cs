...
namespace MHServerEmu.WebFrontend.Network
{
    ...
    internal class GameServiceTaskManager
    {
        ...
        private readonly TaskManager<ServiceMessage.CustomCostumeRegisterResponse> _customCostumeRegisterTaskManager = new();
        ...
        /// <summary>
        /// </summary>
        public async Task<ServiceMessage.CustomCostumeRegisterResponse> RegisterCustomCostumesAsync(ulong sessionId, ulong[] forgedRefs)
        {
            var registerTask = _customCostumeRegisterTaskManager.CreateTask();

            ServiceMessage.CustomCostumeRegisterRequest request = new(registerTask.Id, sessionId, forgedRefs);
            ServerManager.Instance.SendMessageToService(GameServiceType.GameInstance, request);

            ServiceMessage.CustomCostumeRegisterResponse? response = await CompleteTaskWithTimeout(registerTask);

            if (response == null)
                return new(registerTask.Id, (int)HttpStatusCode.RequestTimeout, -1);
        ...
        /// <summary>
        /// </summary>
        public void OnCustomCostumeRegisterResponse(in ServiceMessage.CustomCostumeRegisterResponse response)
        {
            _customCostumeRegisterTaskManager.CompleteTask(response.RequestId, response);
        }
