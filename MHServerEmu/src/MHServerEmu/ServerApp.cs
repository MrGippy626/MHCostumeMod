    ...
    public class ServerApp
    {
        ...
        private bool InitSystems()
        {
            ...
            if (PakFileSystem.Instance.Initialize() == false
                || ProtocolDispatchTable.Instance.Initialize() == false
                || GameDatabase.IsInitialized == false
                || LiveTuningEventScheduler.Instance.Initialize() == false
                || LiveTuningManager.Instance.Initialize() == false
                || CatalogManager.Instance.Initialize() == false
                || IDBManager.Instance.Initialize() == false)
            {
                return false;
            }

            CustomCostumeLoader.ProcessPendingPurges();
