...
namespace DDSLib
{
    ...
    public sealed class DdsFile
    {
        ...
        // ⚑ The public `BitmapSource` property is GONE (decoupling #2 of the WinUI port).
        // It had ZERO consumers outside this file - it existed only to feed the mipmap
        // resize below, which now works on the raw bytes. `BitmapData` already exposes the
        // pixels for anything that wants them, so nothing lost a capability.
        ...
        public void RegenMipMaps(int count)
    {
        ...
        public void GenerateMipMaps(int minMipWidth = 1, int minMipHeight = 1, int mipMax = 0)
        {
            ...
            for (int mipLoop = 1; mipLoop < mipCount; mipLoop++)
            {
                ...
                // Every mip is resampled from the FULL-SIZE image, not from the previous
                // mip. That is what the old WPF chain did (it rebuilt a BitmapSource over
                // largestMipMap each iteration) and it avoids compounding resample error.
                writeSize.MipMap = ResampleRgba(
                    largestMipMap, Width, LargestMipHeight, writeSize.Width, writeSize.Height);

                MipMaps.Add(writeSize);
            }
        }

        // Height derived from the BUFFER, not header.Height - the old RgbaBitmapSource
        // computed it as length / (4 * width), and a header that disagrees with the payload
        // would otherwise read off the end.
        private int LargestMipHeight => largestMipMap.Length / (4 * Width);

        /// <summary>
        /// Downscale tightly packed, NON-premultiplied RGBA.
        ///
        /// ⚑ Replaces a WPF chain (RgbaBitmapSource -> WriteableBitmap -> RenderTargetBitmap
        /// -> ConvertToRgba) that existed only to reach a bilinear/HighQuality resize.
        /// That chain premultiplied on the way in and de-multiplied on the way out, so the
        /// scaling happened in PREMULTIPLIED space - which is the correct way to scale
        /// images with alpha, and why <c>PremultiplyAlpha</c> is left at its default true
        /// here. Turning it off bleeds colour out of fully transparent pixels and shows up
        /// as halos on icon edges.
        /// </summary>
        private static byte[] ResampleRgba(byte[] rgba, int srcW, int srcH, int dstW, int dstH)
        {
            using var img = Image.LoadPixelData<Rgba32>(rgba, srcW, srcH);

            img.Mutate(x => x.Resize(new ResizeOptions
            {
                Size = new Size(dstW, dstH),
                Mode = ResizeMode.Stretch,          // exact dimensions; mips are always /2
                PremultiplyAlpha = true,            // see the note above - matches the old chain
                Sampler = KnownResamplers.Lanczos3,
            }));

            var outBuf = new byte[dstW * dstH * 4];
